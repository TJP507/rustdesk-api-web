<template>
  <PageHeader title="Device Groups" subtitle="Organize devices into logical groups">
  ...
  <FormDrawer v-model="formOpen" :title="form.id ? 'Edit device group' : 'New device group'" :submitting="saving" @submit="submit">
...
import * as deviceGroupApi from '@/api/device_group'

const { state, query, getList, search, del } = useResource(deviceGroupApi, { name: '' })
...
async function submit () {
  ...
  if (!ok) return
  saving.value = true
  const fn = form.id ? deviceGroupApi.update : deviceGroupApi.create
